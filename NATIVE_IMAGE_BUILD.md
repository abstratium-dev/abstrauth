...
You can run it in a kind of test mode as follows:

```
docker run -it --rm -p 4200:8080 -p 9002:9002 \
  --network abstratium \
  -e QUARKUS_DATASOURCE_JDBC_URL=jdbc:mysql://abstratium-mysql:3306/abstrauth \
  -e QUARKUS_DATASOURCE_USERNAME=abstrauth \
  -e QUARKUS_DATASOURCE_PASSWORD=secret \
  ...
  -e OAUTH_GOOGLE_CLIENT_SECRET="${OAUTH_GOOGLE_CLIENT_SECRET}" \
  -e ALLOW_SIGNUP=true \
  -e OAUTH_GOOGLE_REDIRECT_URI="http://localhost:4200/oauth2/callback/google" \
  ...
  -e DEFAULT_ROLES="user,abstratium-abstrauth_manage-clients" \
