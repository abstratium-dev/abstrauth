...
- Issuer: `https://<stage>.abstrauth.abstratium.dev` (stage-specific, e.g. `https://prod.abstrauth.abstratium.dev`)
...
abstrauth's own API (the BFF/resource-server endpoints under `/api/*`) verifies that the token was issued for the abstrauth UI client:

```properties
mp.jwt.verify.audiences=abstratium-abstrauth
```

Tokens issued for other OAuth clients are intended for those clients' own backends; abstrauth is never called with such tokens. Downstream resource servers should enforce the same audience verification using their own client id to reject cross-service token reuse.
...
String token = Jwt.issuer("https://<stage>.abstrauth.abstratium.dev")
...
2. **Issuer Validation**: Tokens must be issued by the stage-specific issuer (e.g. `https://prod.abstrauth.abstratium.dev`)
