...
public class ClientRoleService {
    ...
    /**
     * Update a client role (change the target or role name).
     * Currently only supports updating the role name to another valid role.
     * Verifies that the caller owns the source client and that the new role
     * exists in the target client's allowed roles catalog.
     *
     * @param srcClientId The source client ID
     * @param targetClientId The target client ID
     * @param oldRole The current role name
     * @param newRole The new role name
     * @throws IllegalArgumentException if the role is not found, new role is invalid, or caller doesn't own source client
     * @throws ConflictException if the new role already exists
     */
    @Transactional
    public void updateRole(String srcClientId, String targetClientId, String oldRole, String newRole) {
        // Verify the caller owns the source client
        ownershipVerifier.verifyClientOwnership(srcClientId);

        // Find the existing role
        Optional<ClientRole> existing = findBySrcTargetAndRole(srcClientId, targetClientId, oldRole);
        if (existing.isEmpty()) {
            throw new IllegalArgumentException("Role not found for this client and target");
        }

        // Check if new role already exists (different from old)
        if (!oldRole.equals(newRole)) {
            Optional<ClientRole> conflict = findBySrcTargetAndRole(srcClientId, targetClientId, newRole);
            if (conflict.isPresent()) {
                throw new ConflictException("New role already assigned to this client for the target");
            }

            // Verify the new role exists in the target client's allowed roles
            ClientAllowedRole allowedRole = em.find(ClientAllowedRole.class, new ClientAllowedRole.Id(targetClientId, newRole));
            if (allowedRole == null) {
                throw new IllegalArgumentException("Role '" + newRole + "' is not in the target client's allowed roles catalog");
            }
        }

        ClientRole clientRole = existing.get();
        clientRole.setRole(newRole);
    }

    /**
     * Remove all client roles for a given source client.
     * Used when a client is deleted.
     * Verifies that the caller owns the source client.
     *
     * @param srcClientId The source client ID
     */
    @Transactional
    public void removeAllRolesForSourceClient(String srcClientId) {
        // Verify the caller owns the source client
        ownershipVerifier.verifyClientOwnership(srcClientId);

        List<ClientRole> roles = findBySrcClientId(srcClientId);
    ...
    /**
     * Remove all client roles for a given target client.
     * Used when a target client is deleted or when cleaning up.
     * Note: This does NOT verify ownership - caller must verify.
     *
     * @param targetClientId The target client ID
     */
    @Transactional
    public void removeAllRolesForTargetClient(String targetClientId) {
        List<ClientRole> roles = findByTargetClientId(targetClientId);
        for (ClientRole role : roles) {
            em.remove(role);
        }
    }
