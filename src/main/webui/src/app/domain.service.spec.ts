...
describe('DomainService', () => {
    it('should be true for a subdomain of abstratium.dev', () => {
        expect(buildService('app.abstratium.dev').isAbstratiumDomain).toBe(true);
    });

    it('should be true for localhost', () => {
        expect(buildService('localhost').isAbstratiumDomain).toBe(true);
    });

    it('should be false for a foreign domain', () => {
        expect(buildService('evil.example.com').isAbstratiumDomain).toBe(false);
    });

    it('should be false for a domain that merely ends with abstratium.dev but is not a subdomain', () => {
        expect(buildService('notabstratium.dev').isAbstratiumDomain).toBe(false);
    });
});
